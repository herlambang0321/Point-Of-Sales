...
CREATE OR REPLACE FUNCTION invoice_sales() RETURNS text AS $$
	...
	IF EXISTS(SELECT invoice FROM sales WHERE invoice = 'INV-PENJ' || to_char(CURRENT_DATE, 'YYYYMMDD') || - 1) THEN
		return 'INV-PENJ' || to_char(CURRENT_DATE, 'YYYYMMDD') || - nextval('invoicesales_seq');
	ELSE
		ALTER SEQUENCE invoicesales_seq RESTART WITH 1;
		return 'INV-PENJ' || to_char(CURRENT_DATE, 'YYYYMMDD') || - nextval('invoicesales_seq');
	END IF;
END;
...
-- PRICE UPDATE
CREATE OR REPLACE FUNCTION salesprice_update() RETURNS TRIGGER AS $set_totalprice$
  DECLARE
...
BEFORE INSERT OR UPDATE ON saleitems
    FOR EACH ROW EXECUTE FUNCTION salesprice_update();